...
KeStartAllProcessors()
{
    ...
    while (TRUE)
    {
        ...
        PageTableLoc = ExAllocatePool(NonPagedPool, 0x100000); // A ridiclous megabyte in size for now. Bloaty-Mc Bloat
        /* Load the GDT */
        _sgdt(&BSPGdt.Limit);
        __sidt(&BSPIdt.Limit);
        ...
        DPRINT1("ProcessorState GDT Base loc is %X\n", (ULONG_PTR)APInfo->Gdt);
        DPRINT1("ProcessorState GDT Limit is %X\n", BSPGdt.Limit);
        ...
        ProcessorState.SpecialRegisters.Cr3 = PageTablePhysicalLoc.QuadPart;

        DPRINT1("PageTableLocation in MP is %X\n", PageTablePhysicalLoc.QuadPart);
